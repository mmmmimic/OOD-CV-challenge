...
from datasets import PoseData, rand_bbox, saliency_bbox
...
parser.add_argument('--data', help='path to train dataset', required=True)
parser.add_argument('--val-data', help='path to validation dataset', required=True)
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    # print(out_channels)
    if args.pretrained:
        model = timm.models.swin_base_patch4_window7_224(pretrained=True, num_classes=15)# 15
    else:
        model = timm.models.swin_base_patch4_window7_224(pretrained=False, num_classes=15)
...
def train(train_loader, model, criterion, optimizer, epoch, args):
    ...
    for i, data in enumerate(train_loader):
        ...
        if torch.cuda.is_available():
            ...
            pose = pose.cuda(args.gpu, non_blocking=True)
            angle = angle.cuda(args.gpu, non_blocking=True)
        ...
        # compute output
        output = model(images)
        loss = nn.CrossEntropyLoss()(output[:,:10,...], target_a)*lam + nn.CrossEntropyLoss()(output[:,:10,...], target_b) * (1. - lam) + nn.L1Loss()(torch.sigmoid(output[:,10,...]), pose) + nn.CrossEntropyLoss()(output[:,11:,...], angle)
        output = output[:,:10,...]
    ...
    for nuisance, val_loader in val_loaders:
        ...
        with torch.no_grad():
            ...
            for i, data in enumerate(val_loader):
                ...
                # compute output
                output = model(images)
                loss = nn.CrossEntropyLoss()(output[:,:10,...], target) + nn.L1Loss()(torch.sigmoid(output[:,10,...]), pose)
                output = output[:,:10,...]
...
def save_checkpoint(state, is_best, filename='swin15_checkpoint.pth.tar'):
    torch.save(state, filename)
    if is_best:
        shutil.copyfile(filename, 'swin15_best.pth.tar')
